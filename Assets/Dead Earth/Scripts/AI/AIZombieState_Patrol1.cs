    ...
    public override void OnAnimatorIKUpdated()
    {
       ...
       // Debug.Log("IK Updated " + _nextHeadLookTimer + "    " + Time.frameCount);
        ...
        if (_nextHeadLookTimer > 3.0f)
        {
            ...
            if (Random.value < _randomHeadLookChance)
            {
               // Debug.Log("New head Position");
                _randomLookAtPosition = _zombieStateMachine.transform.position +  Random.onUnitSphere + _zombieStateMachine.transform.forward;
        ...
        { 
            ...
            Vector3 newLookAtTarget = Vector3.Lerp(_zombieStateMachine.currentLookAtPosition, _randomLookTargeter.position, Time.deltaTime);
