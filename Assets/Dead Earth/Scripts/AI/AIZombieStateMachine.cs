...
{
    // Inspector Assigned
    [Header("General Settings")] [SerializeField] [Range(10.0f, 360.0f)]
    float _fov = 50.0f;
    ...
    [Header("Feeding Settings")] [SerializeField] [Range(0.0f, 1.0f)]
    float _satisfaction = 1.0f;

    [SerializeField] float _replenishRate = 0.5f;
    [SerializeField] float _depletionRate = 0.1f;

    [Header("Damage Settings")] [SerializeField] [Range(0, 100)]
    int _lowerBodyDamage = 0;
    ...
    [Header("Target Path Prediction Timing")] [SerializeField] [Range(0, 10)]
    int _predictionBase = 5;

    [SerializeField] [Range(0, 20)] int _predictionIntelligenceScale = 10;

    [Header("Scream Settings")] [SerializeField] [Range(0.0f, 1.0f)]
    ...
    [Header("Reanimation Settings")] [SerializeField]
    bool _reanimate = true;
    ...
    AudioCollection _ragdollCollection = null;
    ...
    public float speedOverride {
        ...
        set {
            /*Debug.Log("Setting Speed Overrode to"+value);*/
    ...
    protected override void Update() {
        ...
        _satisfaction = Mathf.Max(0,
            _satisfaction - ((_depletionRate * Time.deltaTime) / 100.0f) * Mathf.Pow(_speed, 3.0f));
    }
    ...
    protected void UpdateAnimatorDamage() {
        ...
        if (_animator != null) {
            if (_lowerBodyLayer != -1) {
                _animator.SetLayerWeight(_lowerBodyLayer,
                    (_lowerBodyDamage > _limpThreshold && _lowerBodyDamage < _crawlThreshold) ? 1.0f : 0.0f);
            }

            if (_upperBodyLayer != -1) {
                _animator.SetLayerWeight(_upperBodyLayer,
                    (_upperBodyDamage > _upperBodyThreshold && _lowerBodyDamage < _crawlThreshold) ? 1.0f : 0.0f);
            }
            ...
            _animator.SetBool(_lowerBodyDamagedHash,
                (_lowerBodyDamage > _limpThreshold && _lowerBodyDamage < _crawlThreshold) ? true : false);
    ...
    public override void TakeDamage(Vector3 position, Vector3 force, int damage, Rigidbody bodyPart,
        ...
        if (_boneControlType != AIBoneControlType.Animated || isCrawling || IsLayerActive("Cinematic") ||
            attackerLocPos.z < 0) shouldRagdoll = true;
    ...
    protected virtual void LateUpdate() {
        if (_boneControlType == AIBoneControlType.RagdollToAnim) {
            if (Time.time <= _ragdollEndTime + _mecanimTransitionTime) {
                ...
                RaycastHit[] hits = Physics.RaycastAll(newRootPosition + (Vector3.up * 3), Vector3.down, float.MaxValue,
                    _geometryLayers);
                ...
                transform.rotation *=
                    Quaternion.FromToRotation(animatedDirection.normalized, ragdollDirection.normalized);
            }

            // Calculate Interpolation value
            float blendAmount =
                Mathf.Clamp01((Time.time - _ragdollEndTime - _mecanimTransitionTime) / _reanimationBlendTime);
            ...
            foreach (BodyPartSnapshot snapshot in _bodyPartSnapShots) {
                if (snapshot.transform == _rootBone) {
                    snapshot.transform.position =
                        Vector3.Lerp(snapshot.position, snapshot.transform.position, blendAmount);
                }

                snapshot.transform.rotation =
                    Quaternion.Slerp(snapshot.rotation, snapshot.transform.rotation, blendAmount);
            }
