...
public abstract class AIZombieState : AIState
{
    ...
    public override void OnAnimatorUpdated() {
        ...
        if (_stateMachine.useRootPosition && Time.deltaTime > 0.0f)
            _stateMachine.navAgent.velocity =
                (_stateMachine.animator.deltaPosition / Time.deltaTime) * _stateMachine.speedModifier;
        ...
        if (eventType != AITriggerEventType.Exit) {
            ...
            if (other.CompareTag("Player")) {
                ...
                if (curType != AITargetType.Visual_Player ||
                    ...
                    RaycastHit hitInfo;
                    if (_zombieStateMachine.isTrackingPlayer ||
                        ColliderIsVisible(other, out hitInfo, _playerLayerMask)) {
                        // Yep...it's close and in our FOV and we have line of sight so store as the current most dangerous threat
                        _zombieStateMachine.VisualThreat.Set(AITargetType.Visual_Player, other,
                ...
                if (aggrFactor <= _zombieStateMachine.sight && aggrFactor <= _zombieStateMachine.intelligence) {
                    _zombieStateMachine.VisualThreat.Set(AITargetType.Visual_Light, other, other.transform.position,
                        distanceToThreat);
                }
            }
            ...
            if (other.CompareTag("AI Food") && curType != AITargetType.Visual_Player &&
                ...
                if (distanceToThreat < _zombieStateMachine.VisualThreat.distance) {
                    ...
                    if (ColliderIsVisible(other, out hitInfo, _visualLayerMask)) {
                        ...
                        _zombieStateMachine.VisualThreat.Set(AITargetType.Visual_Food, other, other.transform.position,
                            distanceToThreat);
                    }
                }
            }
    ...
    public override void OnAnimatorIKUpdated() {
        ...
        if (_zombieStateMachine.targetType != AITargetType.None) {
            ...
            Vector3 noYLook = new Vector3(_zombieStateMachine.targetPosition.x,
