...
public class IKProceduralStepSystemPlayer : MonoBehaviour
{
    ...
    [SerializeField] protected LayerMask _stepSystemColliderLayer;

    [Tooltip("Amount to scale the Step System data for this NPC instance.")]
    [SerializeField] protected float _stepSystemDataScale = 1;

    [Tooltip("The amount of weight the IK Step System should contribute the final foot positions.")]
    [Range(0,1)]
    [SerializeField] protected float _stepSystemWeight    = 1;
    ...
    protected virtual void Update()
    {
        ...
        _stateMachine.speedModifier = Mathf.Lerp(1.0f, _stepSystemData.GetSpeedScale(_goingUp ? IKStepSystemDirection.up : IKStepSystemDirection.down, _stepSystemDataScale), (_leftIKCooldown + _rightIKCooldown) / 2.0f);
    }
    ...
    protected virtual void OnAnimatorIK(int layerIndex)
    {
        ...
        if (IKType != IKStepSystemType.Simple)
        {
            ...
            leftFootRayOrigin = transform.position - transform.right * _stepSystemData.GetStrideWidth(dir, _stepSystemDataScale);
            rightFootRayOrigin = transform.position + transform.right * _stepSystemData.GetStrideWidth(dir, _stepSystemDataScale);
            ...
            if (leftPlant < 0.9f || !_onLeftStep || _prevLeftRayOrigin.Equals(new Vector3(float.MinValue, float.MinValue, float.MinValue)))
            ...
            if (rightPlant < 0.9f || !_onRightStep || _prevRightRayOrigin.Equals(new Vector3(float.MinValue, float.MinValue, float.MinValue)))
        ...
        // =========================================================================================
        if (Physics.Raycast(leftFootRayOrigin,  Vector3.down, out feetOutHit, _stepSystemData.GetRaycastLength(_stepSystemDataScale), _stepSystemColliderLayer))
        {
                ...
                IKType != IKStepSystemType.AnimatedArcs)
            {
                ...
                LeftIKPosition.y = Mathf.Lerp( !_prevLeftIKY.Equals(float.MaxValue)?_prevLeftIKY : LeftIKPosition.y, LeftIKPosition.y, Time.deltaTime * feetSmoothing);
                ...
                _animator.SetIKRotation(AvatarIKGoal.LeftFoot, Quaternion.FromToRotation(Vector3.up, feetOutHit.normal) * transform.rotation);
            ...
            else
            {
                ...
                LeftIKPosition.y = feetOutHit.point.y + _stepSystemData.GetFootBoneFloorOffset(_stepSystemDataScale) + _animator.GetFloat(_leftFootArcHash) * arcScale;
                LeftIKPosition.y = Mathf.Lerp( !_prevLeftIKY.Equals(float.MaxValue) ? _prevLeftIKY : LeftIKPosition.y, LeftIKPosition.y, Time.deltaTime * feetSmoothing);
                _prevLeftIKY = LeftIKPosition.y;
            }
        ...
        // =========================================================================================
        if (Physics.Raycast(rightFootRayOrigin,  Vector3.down, out feetOutHit, _stepSystemData.GetRaycastLength(_stepSystemDataScale), _stepSystemColliderLayer))
        {
            _goingUp = Vector3.Angle(feetOutHit.transform.right, transform.forward) < 90 ? true : false;
            IKProcessed = true;
                ...
                IKType != IKStepSystemType.AnimatedArcs)
            {
                ...
                RightIKPosition.y = Mathf.Lerp(!_prevRightIKY.Equals(float.MaxValue) ? _prevRightIKY : RightIKPosition.y, RightIKPosition.y, Time.deltaTime * feetSmoothing);
                ...
                _animator.SetIKRotation(AvatarIKGoal.RightFoot, Quaternion.FromToRotation(Vector3.up, feetOutHit.normal) * transform.rotation);
            ...
            else
            {
                ...
                RightIKPosition.y = feetOutHit.point.y + _stepSystemData.GetFootBoneFloorOffset(_stepSystemDataScale) + _animator.GetFloat(_rightFootArcHash) * arcScale;
                RightIKPosition.y = Mathf.Lerp(!_prevRightIKY.Equals(float.MaxValue) ? _prevRightIKY : RightIKPosition.y, RightIKPosition.y, Time.deltaTime * feetSmoothing);
                _prevRightIKY = RightIKPosition.y;
            }
        ...
        float hipDisplacement = leftFootContactHeight < rightFootContactHeight ? leftFootContactHeight : rightFootContactHeight;
       ...
       if (IKProcessed)
            currentBodyPosition.y = Mathf.Lerp(_prevPelvisHeight, hipDisplacement + _stepSystemData.GetPelvisHeight(dir, _stepSystemDataScale), Time.deltaTime * pelvisAdjustmentSpeed);
