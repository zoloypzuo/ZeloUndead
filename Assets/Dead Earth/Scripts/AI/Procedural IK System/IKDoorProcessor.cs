...
{
    // Inspector Assigned
    [Tooltip("The Layer Mask describing which layers should be cast against to find doors. Door colliders should be on their own layer and each have an InteractiveDoor component on it.")]
    [SerializeField] protected LayerMask    _layerMask              = -1;

    [Tooltip("The distance at which a Jogging Zombi should drop down to the approach Speed.")]
    [SerializeField] protected float        _jogSlowDownDistance    = 2.0f;

    [Tooltip("The distance at which a sprinting zombie down down to the approach speed.")]
    [SerializeField] protected float        _sprintSlowDownDistance = 3.25f;

    [Tooltip("The speed to use for final apporach to the door (usually 1.0 the walk speed in the animator.")]
    [SerializeField] protected float        _approachSpeed          = 1;

    [Tooltip("The distance at which the door activation animation will be played.")]
    [SerializeField] protected float        _activateDistance       = 0.51f;

    [Tooltip("The weight to use for tilting the head towards the IK Goal")]
    [Range(0.0f, 1.0f)]
    [SerializeField] float                  _headIKWeight = 1.0f;

    [Tooltip("Show NavMesh Line Casting Lines in the Scene View.")]
    ...
    bool                _processingDoor     = false;    // Are we currently processing a door which includes the pre-animation and the door activation
   ...
   // ---------------------------------------------------------------------------------------------
    void Update()
    {
        ...
        if (!_agent || !_agent.hasPath || _agent.pathPending || _agent.isPathStale || _agent.pathStatus != NavMeshPathStatus.PathComplete)
        {
