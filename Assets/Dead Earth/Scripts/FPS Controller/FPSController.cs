...
public class CurveControlledBob
{
    ...
    public void RegisterEventCallback(float time, CurveControlledBobCallback function,
        ...
        _events.Sort(
            delegate(CurveControlledBobEvent t1, CurveControlledBobEvent t2) { return (t1.Time.CompareTo(t2.Time)); }
        );
    }
...
public class FPSController : MonoBehaviour
{
    ...
    protected void FixedUpdate() {
        ...
        float speed = _isCrouching ? _crouchSpeed :
            _isWalking ? _walkSpeed : Mathf.Lerp(_walkSpeed, _runSpeed, _stamina.value / 100.0f);
        ;
        ...
        RaycastHit hitInfo;
        if (Physics.SphereCast(transform.position, _characterController.radius, Vector3.down, out hitInfo,
        ...
        if (_camera) {
            if (speedXZ.magnitude > 0.01f)
                _camera.transform.localPosition = _localSpaceCameraPos +
                                                  _headBob.GetVectorOffset(
                                                      speedXZ.magnitude * (_isCrouching || _isWalking
                                                          ? 1.0f
                                                          : _runStepLengthen)) + camShakeOffset;
            else
