...
public class CharacterManager : MonoBehaviour
{
    ...
    [Header("Inventory")] [SerializeField] private GameObject _inventoryUI = null;
    ...
    [Header("Arms System")] [SerializeField]
    private Animator _armsAnimator = null;
    ...
    private SharedFloat _health = null;
    ...
    private Dictionary<ScriptableObject, ArmsObject> _armsObjectsDictionary =
        new Dictionary<ScriptableObject, ArmsObject>();

    // Animation Hashes
    private int
        _weaponAnimHash =
            Animator.StringToHash("Weapon Anim"); // The current sub-state machine to play for the selected weapon
    ...
    private int
        _attackAnimHash =
            Animator.StringToHash("Attack Anim"); // Used by machines that have several random attack states

    private int
        _attackTriggerHash = Animator.StringToHash("Attack"); // Used to trigger a transition into an attack state

    private int
        _canSwitchWeaponsHash =
            Animator.StringToHash("Can Switch Weapons"); // Can we switch to a different weapon at the moment

    private int _dualHandedWeaponHash = Animator.StringToHash("Dual Handed Weapon"); // Is the current weapon two handed

    private int
        _dualModeActive =
            Animator.StringToHash("Dual Mode Active"); // Does the current weapon have a dual firing mode that is active
    ...
    private int
        _reloadRepeatHash =
            Animator.StringToHash(
                "Reload Repeat"); // How many times should the reload animation loop (used for partial reload types)
    ...
    private int
        _playerSpeedOverrideHash =
            Animator.StringToHash("Player Speed Override"); // Allows animation to override max speed of player

    private int _clearWeaponHash = Animator.StringToHash("Clear Weapon"); // Hash of Clear Weapon Trigger in animator

    private int
        _dualModeFOVHash =
            Animator.StringToHash("Dual Mode FOV Weight"); // Animation Curve Driven. Used to weight FOV transitions

    private int _crosshairAlphaHash = Animator.StringToHash("Crosshair Alpha"); // Transparency of the crosshair

    public FPSController fpsController {
        get { return _fpsController; }
    }

    public InventoryItemWeapon currentWeapon {
    ...
    void Start() {
        ...
        Dictionary<ScriptableObject, List<ArmsBaseSMB>> stateMachineBehavioursByID =
            new Dictionary<ScriptableObject, List<ArmsBaseSMB>>();
    ...
    public void DisableWeapon_AnimatorCallback() {
        ...
        if (_nextWeapon != null && /*_nextWeapon!=_currentWeapon &&*/ _nextWeaponMountInfo != null) {
            if (_inventory)
                _inventory.DropWeaponItem(_nextWeaponMountInfo.Weapon.weaponType == InventoryWeaponType.TwoHanded
                    ? 1
                    : 0);
        }
    ...
    public void DoDamage(int hitDirection = 0) {
        ...
        if (weapon) {
            ...
            if (weapon.rayRadius > 0.0f) {
                ...
                hits = Physics.SphereCastAll(ray, weapon.rayRadius, weapon.range, _weaponRayLayerMask.value,
                ...
                foreach (RaycastHit potentialHit in hits) {
                    ...
                    if (potentialHit.transform.gameObject.layer == LayerMask.NameToLayer("AI Body Part")) {
                        ...
                        if (Physics.Raycast(siteTestRay, out sightTestHit, 1000, _weaponRayLayerMask.value,
                            QueryTriggerInteraction.Ignore)) {
            ...
            // With a non-radius weapon things are much simpler...just do a raytest for the closest object
                ...
                isSomethingHit = Physics.Raycast(ray, out hit, weapon.range, _weaponRayLayerMask.value,
                    QueryTriggerInteraction.Ignore);
            ...
            if (isSomethingHit && hit.rigidbody) {
                ...
                if (stateMachine) {
                    ...
                    float damage = weapon.GetAttentuatedDamage(hit.rigidbody.tag, hit.distance) *
                                   (wmi == null ? 1 : wmi.Condition / 100.0f);
                ...
                if (!(priorityObject.GetType() == typeof(CollectableWeapon) && !_canSwitchWeapons)) {
                    if (Input.GetButtonDown("Use")) {
                        ...
                        if (_currentWeapon && _inventory)
                            _availableAmmo = _inventory.GetAvailableAmmo(_currentWeapon.ammo,
                                AmmoAmountRequestType.NoWeaponAmmo);
                    }
                }
            }
        }
        ...
        if ((_inventoryUI && !_inventoryUI.activeSelf) || !_inventoryUI) {
            ...
            if (_armsAnimator) {
                ...
                if (_canSwitchWeapons) {
                    ...
                    else if (Input.GetButtonDown("Dual Handed Mount")) mountIndex = 1;
                }
                ...
                if (Input.GetButtonDown("Fire1")) {
                    ...
                    if (_currentWeapon) {
                        ...
                        if ((_currentWeapon.weaponFeedType == InventoryWeaponFeedType.Ammunition &&
                             currWMI.InGunRounds > 0 && currWMI.Condition > 0) ||
                            _currentWeapon.weaponFeedType != InventoryWeaponFeedType.Ammunition) {
                    ...
                    if (currWMI != null && currWMI.InGunRounds < _currentWeapon.ammoCapacity) {
                        ...
                        if (_inventory.IsReloadAvailable(mountIndex)) {
                            ...
                            if (_currentWeapon.reloadType == InventoryWeaponReloadType.Partial)
                                _armsAnimator.SetInteger(_reloadRepeatHash,
                                    _currentWeapon.ammoCapacity - currWMI.InGunRounds);
                            else
    ...
    public void ActivateFlashlightLight_AnimatorCallback(bool enableLight, FlashlightType type) {
        ...
        else if (_secondaryFlashLight != null) {
